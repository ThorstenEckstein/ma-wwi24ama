...
. Entscheide selbst über die Datentypen der Attribute, aber mache sie "nicht öffentlich".

. Erstelle zudem Methoden für das Auslesen und Setzen der Attribute (get-/setName, get-/setAge)

. Schreibe hier einen einfachen Test: Setze konkrete Werte für Name und Alter (set), überprüfe dann,
