...
Den "agilen" Entwicklungsprozess (agile Software Development) kann am besten mittels Grafik erläutert werden:
...
* Entwicklungsumgebung `IntelliJ` und
...
Ggf. muss diese Version noch installiert werden, wobei die Versionsnummern ("Minor") _hinter_ der Hauptversionsnummer 21 ("Major") nicht so wichtig sind.
...
Kleinere Abweichungen in der Version sollten kein Problem sein, die letzten Versionen sind in aller Regel untereinander kompatibel. Bei Problemen versucht man es einfach mit der aktuellsten IDE Version.
...
.. `Clone Repository`, in älteren Versionen `Get from VCS` anklicken (VCS = Version Control System, `Git`)  image:images/intellij-clone-repository.png[align="center", width="55%"] oder image:images/intellij-start-get-from-vcs.png[align="center"]
