...
Der Code für den Kurs wurde mit folgender Version erstellt, die installiert sein muss (falls noch nicht vorhanden, bitte installieren):
 ...
 IntelliJ IDEA 2024.3.1 (Community oder Ultimate Edition)
...
*Bundled PlugIns*::
Mit IntelliJ werden bereits diverse PlugIns *automatisch mitinstalliert*, zu diesen gehören die folgenden. Sie sind für das Seminar erforderlich und sollen anfangs mal geprüft werden:
...
Allerdings kommt im Kurs nur ein kleiner Teil hiervon zum Einsatz, lediglich das erstmalige *Downloaden* (`clone`) & die regelmäßige *Aktualisierung* (`pull`) der lokalen Dateien wird benötigt:
