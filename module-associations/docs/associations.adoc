...
public class Document {
    prrivate TableOfContent toc;
    public Document(TableOfContent inhaltsverzeichnis) { <1>
        this.toc = inhaltsverzeichnis
    }
}
----

<1> Starkes *Indiz* für existenzielle Abhängigkeit: Das direkte Setzen der abhängigen KLasse und (oft auch) deren Initialisierung im oder über den _Konstruktor_!
...
* als *komplexes Attribut* -> *(2)* realisiert sind oder
* nur als *primitiver "Referenz"-Wert*, z.B. die `id` der anderen Klasse -> *(3)*

vorhanden sind (_auch die "besitzende" Klasse hat in aller Regel selbst eine ID -> *(1)*_).
