...
Es gibt *zwei Hauptwege*, wie Abstraktion in Java umgesetzt wird, nämlich

. Interfaces und
. abstrakte Klassen

[[about_abstractclasses]]
*Ziele* von Abstraktion: Die *Abstraktion* als eines der wichtigsten *Kernkonzepte* in Java wird vor allem eingesetzt, um verschiedene Erfordernisse umzusetzen. Dazu gehören u.a.

* Ermöglichen, auf einem höheren Abstraktionsniveau zu programmieren (z. B. „etwas fährt“, ohne wie genau dies geschieht zu kennen).
* Wiederverwendbarkeit und Wartbarkeit des Codes verbessern.
* Implementierungsdetails verstecken und eine klare Programmierschnittstelle (API = Application Programming Interface) für andere Entwickler bieten.

====
{icon-question} Was ist eine Programmierschnittstelle (API)?
====
