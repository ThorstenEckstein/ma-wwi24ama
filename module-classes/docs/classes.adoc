...
image::../../module-funcprogramming/docs/images/paradigms.png[align=center]
...
Eine *Signatur* einer Java Klasse stellt quasi den Rahmen des komplexen Datentyps dar und nutze das Schlüsselwort `class`:
...
Ein *Konstruktor* ist eine spezielle *Methode* in einer Klasse, die beim *Erzeugen eines Objekts* aufgerufen wird (wie andere Methoden auch). Er dient/sie dienen dazu, das Objekt zu initialisieren, also Anfangswerte für Felder zu setzen oder notwendige Vorbereitungen zu treffen. In Java hat der Konstruktor immer denselben Namen wie die umgebende Klasse und besitzt _keinen_ Rückgabewert.
...
NOTE: _Der Aufruf `super()` wird später in der Betrachtung der *Vererbung* näher erläutert._

Eine Klasse mit einem *Konstruktor*, der ein Argument entgegennimmt:
...
IMPORTANT: _Derjenige Konstruktor einer Klasse, der keine Argumente hat, ist der sogenannte *Standard-Konstruktor* (engl. default c'tor). Dieser muss nicht explizit geschrieben werden, denn jede Klasse in Java hat automatisch einen Standard-Konstruktor, aber nur solange es_ keine anderen, _d.h. solche mit Argumenten, gibt!_
...
public class Person {
    ...
    // Eigenschaft mit Sichtbarkeit, Datentyp und Variablenname
    public String name;

}
----

IMPORTANT: _Die "Lesart" dieses Beispiels lautet: Jede `Person` [.underline]#hat# eine Eigenschaft `Name`_
...
_Frage: Fällt bei den Implementierungen aus der Übung irgendetwas besonders auf?_
...
====
TIP: {prompt} "_Erzeuge eine oder beide Klassen mithilfe einer KI (GenAI). Wie muss der Prompt formuliert werden, gibt es Besonderheiten zu beachten? Vergleiche die KI-Implementierung mit der manuell implementierten Klasse._"
====
...
[[about_inheritance]]
Bei der *Vererbung* in Java wird zwischen _zwei zueinander in Beziehung stehenden_ Klassen, nämlich
...
Die _Subklasse_, auch Kind- oder Unterklasse bezeichnet, bekommt von ihrer Elternklasse sämtliche Attribute und Methoden vererbt, die nicht `private` (wird vertieft an LV-Tag 3, Kapitel "Sichtbarkeiten") sind.

Insbesondere kann die Subklasse um eigene Attribute und Methoden *erweitert* werden. Man spricht dann von einer

[subs=normal]
 *Spezialisierung*

der Superklasse durch die Subklasse.
...
*Objekterzeugung in einer Klassenhierarchie*

Wenn Klassen instanziiert werden, also Objekte aus Klassen erzeugt werden, dann ist die Frage, was _bei diesem Prozess mit etwaigen Eltern- oder Kindklassen geschieht_, d.h. wie die Konstruktoren der Klassenhierarchie aufgerufen werden. Hier kommt das *Schlüsselwort*

`super()`

ins Spiel. Mit der Methode `super()` können Elemente der Elternklasse explizit aufgerufen bzw. genutzt werden. Das wird regelmäßig in Konstruktoren gemacht.

Die Methode `super()` wird also in Konstruktoren verwendet, um den passenden Konstruktor einer Elternklasse aufzurufen.
...
Wenn eine Klasse von einer anderen Klasse erbt (mittels Schlüsselwort `extends`), wird beim Erstellen eines Objekts automatisch der passende Konstruktor der Oberklasse aufgerufen.

Mit `super()` kann also gesteuert werden, welcher Konstruktor der Oberklasse genau aufgerufen werden soll.

[source, java]
----
public class Elternklasse {

    public Elternklasse() { <1>
        // leerer Standard-Konstruktor
    }
    public Elternklasse(String parameter) { <2>
        // Inhalt des Konstruktors mit einem String-Parameter
    }
}
...
----
<1> Standard-Konstruktor der Elternklasse
<2> 1-Parameter-Konstruktor der Elternklasse
<3> Aufruf des Standard-Konstruktors der Elternklasse
<4> Aufruf des Eltern-Konstruktors mit EINEM String-Parameter


*`this()`*

Ähnlich wie zuvor gibt es das Schlüsselwort bzw. die Methode `this()`. Sie wird verwendet, um andere Elemente *derselben* Klasse aufzurufen, hier am Beispiel von Konstruktoren-Aufrufen.

Anders als `super()`, das zur Kommunikation mit der Oberklasse dient, ermöglicht `this()` also die Wiederverwendung von Feldern, Methoden oder Konstruktoren innerhalb der _eigenen_ Klasse.

Ein Konstruktor kann also z.B. mit `this()` einen anderen Konstruktor derselben Klasse aufrufen, um gemeinsame Initialisierungslogik zu teilen. Dies ist besonders nützlich bei Klassen mit mehreren Konstruktoren, die sich ergänzende Aufgaben ausführen.
...
====
TIP: {prompt} "_Erstelle eine Beispiel-Klassenhierarchie in Java. Wähle ein Beispiel aus der dem Kontext "Deutsche Bahn" für Klassen. Das Klassenmodell soll jeweils eine Klasse, eine abstrakte Klasse und ein Interface beinhalten._"
====
