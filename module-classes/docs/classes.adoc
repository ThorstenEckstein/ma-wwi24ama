...
* Eigenschaft (Feld, engl. _field_),
* Konstruktor (engl. _constructor_) und
* Verhaltensweise (Methode, engl. _method_):
...
In Java spielen die Konstruktoren eine wichtige Rolle bei der *Erzeugung von Objekten*. Sie sind spezielle Methoden, die aufgerufen werden, wenn ein neues Objekt erstellt wird, und ermöglichen es, das Objekt mit bestimmten Werten oder Eigenschaften zu initialisieren. Dazu wird das Schlüsselwort `new` benutzt (siehe unten):
...
Klassen besitzen meistens auch "Eigenschaften" in Form von einer oder mehreren *Feldern*. Zunächst ein Beispiel:
...
IMPORTANT: Die "Lesart" dieses Beispiels lautet: Jede `Person` hat eine Eigenschaft `Name`


Klassen besitzen darüber hinaus meistens auch "Verhalten" in Form von einer oder mehreren *Methoden*. Am Beispiel:
...
Von *Überladung* wird gesprochen, wenn eine (Unter-)Klasse neues bzw. zusätzliches Verhalten *hinzufügt*.
...
[source,java]
----
include::{mod-lnk-src}/demo/override/DefaultDisplay.java[lines="5..10"]
----

[source,java]
----
include::{mod-lnk-src}/demo/override/PlatformDisplay.java[lines="3..10"]
----
