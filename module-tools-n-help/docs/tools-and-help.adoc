  ...
  {icon-dir} <Modul-Wurzel>
    ...
    {icon-dir} src         -> Wurzel für den gesamten Sourcecode
      {icon-dir} main      -> Wurzel für den "produktiven" Sourcecode
        {icon-dir} java ...
      {icon-dir-test} [lime]#test#      -> [lime]#Wurzel für den Test-Sourcecode#
        {icon-dir-test} [lime]#java# ...
    {icon-dir-target} [red]#target#      -> [red]#Kompilierte Klassen#
----
...
{icon-dir} D:\Projekte\JavaProgammierung
  {icon-dir} module-classes
    ...
    {icon-dir} src
      {icon-dir} main
        {icon-dir} java              -> ab hier "Package" Verzeichnisse
          {icon-dir-package} de
...
== Einsatz von Hilfsmitteln/Tools in der IDE

IntelliJ bietet eine Reihe von Hilfsmitteln im Standard und über PlugIns an, die bei der Implementierung von Code unterstützen können.

Innerhalb der *LV* gelten folgende *Regeln*:

. Die Nutzung von *Code-Generierung* ist direkter _Teil der LV_ und wird nach und nach intensiver genutzt/geübt. Zu Beginn der LV zunächst nicht sinnvoll, gegen Ende aber erlaubt bzw. sogar erwünscht

. Die Nutzung von *Auto-Completion* ist standardmäßig in der IDE _aktiviert_ und kann genutzt werden. Sie kann, z.B. zu Übungszwecken, abgeschaltet werden. Grundsätzlich hilft dieses Feature, Fehler zu vermeiden

. *KI-Tools* werden in dieser LV _nicht_ wirklich eingesetzt, da der Fokus auf dem Erlernen der Grundlagen der Javaprogrammierung liegt. Am Ende der LV ist der Einsatz von KI-Tools aber Gegenstand eines Moduls und wird näher betrachtet

Sowie *Empfehlungen*:

. *Auto-Completion* auch mal abschalten, um sich selbst zu fordern
. *Auto-Completion* bewusst "lesen", um den Vorschlag zu verstehen
. *Auto-Completion* nur nutzen, wenn man den Vorschlag versteht
. *Code-Generierung* üben, aber auch hier gilt: Gezielt einsetzen und Verstehen, was generiert wird.
. *KI-Tools* zunächst nicht für die Code-Generierung nutzen, aber z.B. zur Erklärung von Code, Algorithmen oder Prinzipien
. Es ist wichtig, zunächst die *Grundlagen* zu verinnerlichen, sich selbst zu erarbeiten und praktisch zu üben! Erst dann kann eine KI-Antwort bewertet werden

In der *Prüfung*:

. *Auto-Completion* bleibt standardmäßig aktiviert und kann genutzt werden.
. *Code-Generierung* darf nur auf Anforderung bzw. bei Bedarf mit Rücksprache der Prüfer:Innen genutzt werden.
. *KI-Tools* sind in der Prüfung nicht erlaubt.

CAUTION: [red]#_Es kann jederzeit Gegenstand der Prüfung sein/werden, dass Kandidat:Innen Quellcode_ OHNE _die Nutzung der obigen Hilfsmittel zu schreiben, daher wird empfohlen, regelmäßig Code selbst zu schreiben und sich nicht zu stark unterstützen zu lassen_#
...
|===
| An der Cursorposition _Rechts-Klick_ > `Generate ...`
| An der Cursorposition > _IntelliJ-Menü_ > `Code` > `Generate ...`
...
=== Automatische Code-Ergänzung (Autocompletion)
...
| _Anzeige von möglichen Methoden einer Instanz_
| _Auswahlmöglichkeiten für nutzbare Typen, hier Klassen_
...
Der hier eingesetzte *GitHub Copilot* ist ein KI‑gestützter "Pair Programmer", der direkt im Editor Code‑ und Kommentarvorschläge liefert. Er basiert auf großen Sprachmodellen, die auf öffentlich zugänglichem Quellcode und natürlicher Sprache trainiert wurden und kontextabhängige Vorschläge für einzelne Zeilen, ganze Methoden oder Code‑Blöcke machen können.

. Integration: Plugins für JetBrains (IntelliJ) oder Visual Studio Code und weitere Editoren.
. Funktionen: Inline‑Vervollständigungen, Generierung von Methoden oder Tests, Vorschläge für Refactorings.
. Nutzungshinweis: Copilot kann die Produktivität steigern und als Lernhilfe dienen, ersetzt aber nicht das Verständnis des Codes. Alle Vorschläge sollten unbedingt geprüft und ggf. anpasst werden.
